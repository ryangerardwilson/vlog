...
FILENAME="blog-linux-x64.tar.gz"
...
write_public_launcher() {
  ...
  elif [[ -f "$PUBLIC_LAUNCHER" ]] && ! grep -Fq '# Managed by blog local-bin launcher' "$PUBLIC_LAUNCHER" 2>/dev/null; then
...
#!/usr/bin/env bash
# Managed by blog local-bin launcher
set -euo pipefail
